...
const getPopulation = () =>
  countries.reduce(
    (populacaoTotal, pais) => populacaoTotal + pais.population,
    0
  );
...
const getTotalArea = () =>
  countries.reduce((areaTotal, pais) => areaTotal + pais.area, 0);
...
const longestName = () =>
  countries.reduce((maiorNome, pais) =>
    maiorNome.name.length > pais.name.length ? maiorNome : pais
  );
...
expectedResult = 20;
const countA = () =>
  names.reduce((counterA, name) => counterA + name.match(/[Aa]/g).length, 0);
...
const studentAverage = () => students.map((name, index) => ({ name, average: grades[index]
    .reduce( (media,nota,undefined,notas) => media+nota/notas.length,0).toFixed(1) }));
console.log(studentAverage(), '===', expectedResult);
